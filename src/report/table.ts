...
function buildHeader(columns: any) {
  const buildCells = () =>
    columns
      .map((col: any, i: any) => getCell(getAlign(col), col.title))
      .join("");
...
function buildRows(rows: any, columns: any) {
  const buildCells = (cells: any) =>
    cells.map((cell: any, i: any) => getCell(getAlign(columns[i]), cell));
  const buildRow = (row: any) =>
    rowTemplate({ cells: buildCells(row).join("") });
